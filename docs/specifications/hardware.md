...
- SH1106 OLEDはI2C0に専用接続する。SDAはGP0、SCLはGP1とする。
- ADS1015とMCP23017はI2C1を共有する。SDAはGP2、SCLはGP3とする。
- 各バスの速度は400 kHz（Fast-mode）、信号レベルは3.3 Vとする。
- ADS1015の7 bit I2Cアドレスは`0x48`とする。SH1106 OLEDのアドレスは`0x3C`とする。
- MCP23017の7 bit I2Cアドレスは`0x20`とする。アドレス端子A0〜A2はGNDへ接続する。
- 各I2CバスのSDA/SCLには3.3 Vへのプルアップを1組だけ配置する。モジュール側のプルアップを有効化する場合、基板側には重複して配置しない。
- ADS1015の`ALERT/RDY`は3.3 Vへのプルアップ抵抗を設け、GP6へ接続する。変換完了パルスの立下りエッジで割り込みを発生させる。
...
MCP23017でフットスイッチ入力を扱い、入力系はI2C1へ集約する。OLEDはI2C0へ分離するため、将来のデュアルコア構成でUI処理と入力処理がI2Cバスを共有しない。割り当ては[ピン割り当て](pin_assignment.md)を参照する。GP7およびGP26は将来の物理入力または出力に使える。
