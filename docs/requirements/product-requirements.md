...
- 電源投入時はPreset 1を使用し、プリセット有効化や起動だけではMIDIを送信しない。
...
- 設定メニューには、プリセット設定、本体設定、保存して終了、保存せず終了、工場出荷状態リセットを設ける。
...
- EXP入力は6.35 mm TRSジャックを使用し、外部I2C ADCのADS1015で読み取る。ADS1015のData Ready割り込みで変換完了を通知する。
- フットスイッチ入力はI2C GPIOエキスパンダーのMCP23017で読み取る。
