...
初期状態では実機確認用に、USB MIDIデバイスが接続されるとチャネル1のProgram Change 0と
Control Change 1（値64）を1秒ごとに交互送信します。MIDIモニターで受信を確認できます。
確認後は次のように無効化してください。

```bash
cmake --preset debug -DUSB_MIDI_PEDAL_USB_MIDI_DIAGNOSTIC_SEND=OFF
cmake --build --preset debug
```
