...
- Issue作成時はリポジトリの既存マイルストーンを確認し、作業内容に該当するマイルストーンへIssueを割り当ててください。該当するマイルストーンがない、または複数候補から選べない場合は、ユーザーへ確認してください。
- 日本語または改行を含むIssueおよびPull Request本文をCLIから渡す場合は、UTF-8で保存したMarkdownファイルを`--body-file`で指定してください。`--body`へ直接渡す方法、文字列`\n`、PowerShellの文字列エスケープには依存しません。
- 本文ファイルには実際の改行を使用してください。作成または更新後は、`gh issue view --json body`または`gh pr view --json body`で、日本語・改行・見出し・箇条書きが正しく保存されていることを確認してください。
